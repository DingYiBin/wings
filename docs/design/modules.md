...
| `skill_view` | `builtin/skill_view.py` | P1 | 加载 skill 内容 |
| `agent` | `builtin/agent.py` | P1 | 启动子 agent |
...
| 6b | agent/subagent | ✅ | `subagent.py` (AgentTypeSpec, 3 builtin types, run_subagent), `tools/builtin/agent.py` | 24 |
| 7 | config | ✅ | `settings.py` (JSON, ProviderConfig w/ thinking/max_tokens) | — |
| 8 | cli | ✅ | `main.py` (chat + run, slash commands, tool display, permission UI, ctrl+o pager) | — |
| 9a | skills | ✅ | `types.py`, `loader.py`, `injector.py`, `builtin_data.py`, `builtin/` SKILL.md | 31 |
| 9b+ | hooks, memory, plugins, MCP | — | 后续迭代 | — |
...
│   ├── loop.py         # AgentLoop (per-call model selection, permission sync, subagent events)
│   └── subagent.py     # AgentTypeSpec, BUILTIN_AGENT_TYPES, run_subagent()
├── config/             # Phase 7  ✅
│   ├── __init__.py
...
- CLI 截断统一为 1 行 + `… +N lines (ctrl+o)`，按 ctrl+o 调用 `$PAGER` 展开完整内容

#### 24. Subagent 系统（Phase 6b）

参考 claude-code 的 AgentTool 设计，实现了 subagent 委托执行：

**核心设计**:
- 复用 `AgentLoop`：每个 subagent 创建新的 `AgentLoop` 实例（只存引用，成本为零），无需写第二套循环代码
- Agent 作为标准 `@tool` 工具：LLM 调用 `agent(subagent_type="explore", description="...", prompt="...")`
- 3 种内置类型：`general`（全部工具）、`explore`（只读，Read/Glob/Grep/SkillView）、`plan`（只读，禁止 Write/Edit）
- 每种类型对应独立路由池 `subagent/<type>`

**关键设计决策**:
- `make_agent_tool()` 工厂函数用闭包注入依赖（QueryEngine, ModelRegistry 等），避免引入 DI 容器
- `ToolContext.event_callback` 字段让 subagent 事件回流到 parent AgentLoop
- Subagent 权限全部预授权（`PermissionRules` 预填所有过滤后工具），不触发交互式 prompt
- 大小写不敏感的类型匹配（模型可能传 "Explore" 而非 "explore"）

**工具过滤**: `_filter_tools_for_agent()` 返回新 `ToolRegistry`，不修改 parent。过滤链：allowlist → disallowed → read_only → always remove "agent"

**CLI 显示**: SubAgentStart 用 `╭─ Agent(type) ─ description` 边框，子 agent 文本缩进显示，SubAgentEnd 用 `╰─ Agent(type) done` 收尾

**Tool 结果简化**: 参考 opensquilla CLI 的 1 行预览设计，tool 结果默认只显示首行（≤120 chars），多行结果存储全文，ctrl+o 用 `$PAGER` 展开
